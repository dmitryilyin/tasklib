  ...
  class RSpecAction < Action
    ...
    def initialize(task, action, spec = nil)
      spec = case action.to_sym
               when :pre then
                 Tasklib::Config[:spec_pre]
               when :post then
                 Tasklib::Config[:spec_post]
               else
    ...
    def rspec_command
      rspec_command = 'rspec'
      report_format = Tasklib::Config[:report_format]
      case report_format
        when 'xunit' then
    ...
    # @return[Fixnum]
    def call
      Tasklib::Utils.debug "Start #{self.class} with action '#{action.to_s}' and '#{path}' spec"
      unless exists?
        report_no
        return 0
      end
